...
export default function Editor({
  ...
  isNoteInfoFill,
  setIsNoteInfoFill,
  setTextIntoWorryPopup,
  postNotes,
  putNote,
  noteClickId,
  choosedNoteBgColor,
  choosedNoteIsDone,
  originalChoosedNoteDescription
}) {
  ...
  let {userId} = useUserContext()
   ...
   function onSaveNoteHandler() {
    ...
    // const response = await fetch(`${process.env.REACT_APP_FIREBASE_DATABASE_URL}/${userId}/.json`, {
    //   method: "POST",
  ...
  function saveNoteBtnHandler(){
    if(!!titleFieldRef.current.value.length === false || !!descriptionFieldRef.current.value.length === false){
      setTextIntoWorryPopup("Please, add a title or description to your add!")
